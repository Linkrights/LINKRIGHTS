...
import { formatDate, getMessages, type Locale } from '@/lib/i18n';
import type { AskApiResponse } from '@/lib/types';
...
export function AskClient({
  ...
  async function ask(text: string) {
    ...
    try {
      ...
      setResult({ ok: false, error: 'server', fallback: fallbackLinks });
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
      ...
      <div ref={resultRef} tabIndex={-1} aria-live="polite" className="mt-8 space-y-5 outline-none">
        {loading && (
        ...
        {errorMessage && (
          <div className="lr-card border-[var(--color-warm-500)] bg-warm-100 p-5">
            ...
            {result && !result.ok && result.fallback && result.fallback.length > 0 && (
              <>
        ...
        {result?.ok && result.mode === 'emergency' && (
          <div className="lr-card p-5">
            ...
            <button
              ...
              className="lr-btn lr-btn-ghost mt-3"
            >
              {t.ask.newQuestion}
            </button>
          </div>
